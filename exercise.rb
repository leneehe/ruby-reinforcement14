...
# 1
result[:response][:results].each do |x|
  x[:views] = 0
end

# 2
def read_article(result)
  result[:response][:results].sample[:views] += 1
end
...
# 3
def display_views(result)
  result[:response][:results].each { |x|
    puts "title: #{x[:webTitle]}"
    puts "views: #{x[:views]} "
    puts "-" * 5
  }
end

display_views(result)
